...
const Splash: Component = ({duration,children}) => {
    setSplashDuration(duration || 1000);

    return (
        <>
            <Presence >
            <Show when={!splashDone()}>
            <Motion.div class={styles.Splash} initial={{opacity:1}} exit={{opacity: [1,0],duration: 0.3}} transition={{duration: 1, easing: "ease-in-out"}}>
